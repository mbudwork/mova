...
const ru: LandingCopy = {
  ...
  momentFootnote: 'Озвучка появится вместе с полным курсом — текст пока читаем после ответа.',
  momentCta: 'А сколько поймёшь ты? Пройти тест из 7 команд',
  ...
  faq: [
    ...
    {
      q: 'Чем это отличается от переводчика?',
      a: 'Переводчик помогает перевести фразу, когда есть время. MOVA тренирует понимать команду в момент, когда она уже прозвучала.',
    },
    {
...
const uk: LandingCopy = {
  ...
  momentFootnote: 'Озвучення з’явиться разом із повним курсом — поки читаємо текст після відповіді.',
  momentCta: 'А скільки зрозумієш ти? Пройти тест із 7 команд',
