 ...
 * оплаты, а приложение просит задать пароль на первом же экране.
 *
...
export async function findOrCreateBuyer(
  ...
  const { data, error } = await supabase.auth.admin.createUser({
    ...
    email_confirm: true,
    /*
      Метка, по которой приложение потом требует задать пароль. Без неё
      покупатель остался бы с аккаунтом, в который можно войти только по
      куке: очистил браузер или взял другой телефон — и всё, вход только
      через письмо. Ставится только новым, созданным оплатой: у тех, кто
      регистрировался руками, пароль уже есть, и дёргать их незачем.
    */
