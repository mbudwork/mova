  ...
  */
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      /*
        custom_text здесь был и сломал боевую оплату.

        Подпись «почта станет логином» рядом с кнопкой Stripe — идея хорошая,
        но Managed Payments запрещает любые чужие вставки в свою страницу: в
        этом режиме Stripe сам продавец записи и сам отвечает за то, что там
        написано. Ответ на создание сессии:
        «custom_text cannot be used with Managed Payments».

        Обойти можно было бы через managed_payments[enabled]=false, но тогда
        НДС по месту покупателя ложится на MBUD: регистрация OSS, отчётность,
        ставки по странам ЕС. Отдавать это ради одной подписи — плохой обмен.
        Предупреждение остаётся на нашей странице до перехода и на экране
        «Доступ открыт» после оплаты.
      */
