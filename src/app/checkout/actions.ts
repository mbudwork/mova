  ...
  */
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      /*
        Надпись на самой странице Stripe, рядом с кнопкой оплаты.

        Предупреждение «почта станет логином» у нас стоит на своей странице,
        но поле для адреса — здесь, на чужой. Человек уже переключился в режим
        оплаты и о нашем экране не помнит: вводит адрес как реквизит платежа.
        Опечатка в домене превращается в аккаунт, куда не войти и который не
        восстановить, — чинится только вручную через базу.

        custom_text ограничен 1200 символами и не поддерживает разметку.
      */
      custom_text: {
        submit: {
          message:
            locale === 'uk'
              ? 'Пошта із цієї форми стане твоїм логіном у MOVA. Вписуй ту, якою користуєшся: на неї прийде підтвердження і за нею ти входитимеш у курс.'
              : 'Почта из этой формы станет твоим логином в MOVA. Впиши ту, которой пользуешься: на неё придёт подтверждение, и по ней ты будешь входить в курс.',
        },
      },
      metadata: {
